...
public class VisionShooterCalculator {
    ...
    public static double GetSpeakerShooterAngle()
    {
        if (vision.hasValidData())
        {
            ...
            return Math.toDegrees(Math.atan2(heightDifferenceBetweenShooterAndSpeakerTargetPosition, horizontalDistanceToAprilTag)) + Math.pow(horizontalDistanceToAprilTag, 2) * C;
        }
